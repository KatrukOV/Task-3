...
public class SAXParser extends AbstractParser {
	...
	class SaxHandler implements ContentHandler {
		...
		public void endElement(String uri, String localName, String qName) throws SAXException {
			if (isOptions) { device.setOptions(options); }
			isOptions = false;
			temp = "";
		}
		...
		public void characters(char[] ch, int start, int length) throws SAXException {
			switch (temp) {
				case NAME:
					device.setName(new String(ch, start, length));										break;
				case PRICE:
					device.setPrice(Double.valueOf(new String(ch, start, length)));						break;
				case ORIGIN:
					device.setOrigin(new String(ch, start, length));									break;
				case COMPONENTS_GROUP:
					device.setComponentsGroup(ComponentsGroup.valueOf(new String(ch, start, length)));	break;
				case PERIPHERALS:
					options.setPeripherals(Boolean.valueOf(new String(ch, start, length)));				break;
				case ENERGY_CONSUMPTION:
					options.setEnergyConsumption(Integer.valueOf(new String(ch, start, length)));		break;
				case PRESENT_COOLER:
					options.setPresenceCooler(Boolean.valueOf(new String(ch, start, length)));			break;
				case PORTS:
